...
FluxKart.com is deadpan serious about their customer experience. There is nothing more important than rewarding their loyal customers and giving a personalised experience. 
To do this, FluxKart decides to integrate our platform suppose Platform-A into their platform. Platform-A collects contact details from shoppers for a personalised customer experience. 

However, given Doc's modus operandi, Platform-A faces a unique challenge: linking different orders made with different contact information to the same person.

Platform-A needs a way to identify and keep track of a customer's identity across multiple purchases.

We know that orders on FluxKart.com will always have either an **`email`** or **`phoneNumber`** in the checkout event.

Platform-A keeps track of the collected contact information in a relational database table named **`Contact`.**
...
We are required to design a web service with an endpoint **`/identify`** that will receive HTTP POST requests with JSON body of the following format:
...
Note: Above request has phonenumber and email from two different existing contacts
...
Note: Oldest contact remained primary
